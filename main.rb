...
if google_credentials && google_sheet_id
  ...
  puts "   ❗ 구글 시트 설정 누락"
  puts "      .env 파일에 GOOGLE_CREDENTIALS_PATH와 GOOGLE_SHEET_ID를 설정하세요"
end
...
begin
  ...
  if File.exist?(google_credentials)
    ...
    if missing_sheets.empty?
      puts "   ✅ 모든 필수 워크시트 확인 완료"
    else
      puts "   ❗ 누락된 워크시트: #{missing_sheets.join(', ')}"
      puts "      구글 시트에서 해당 워크시트를 생성해주세요"
    end
