...
module HouseScoreUpdater
  module_function
  # ⬆️ 이 한 줄로 “HouseScoreUpdater.update_house_scores” 형태로 호출 가능
  ...
  def update_house_scores(sheet_manager)
    ...
    # 1️⃣ 사용자 시트 데이터 읽기
    users = sheet_manager.read("사용자!A:I")
    ...
    if house_idx.nil? || score_idx.nil?
      puts "[ERROR] '기숙사' 또는 '개별 기숙사 점수' 열이 없습니다."
      return
    end
    ...
    houses_sheet.each_with_index do |row, i|
      ...
      range = "기숙사!B#{i + 1}"
      sheet_manager.write(range, [[total_score]])
