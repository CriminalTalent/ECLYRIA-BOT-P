...
module MastodonClient
  ...
  def self.listen_mentions
    begin
      ...
      if mentions.any?
        ...
        puts "📚 새로운 학생 호출 #{mentions.size}개 도착!"
  ...
  def self.reply(mention, message)
    begin
      ...
      if messages.length == 1
        ...
        puts "✅ @#{acct}에게 교무 응답 완료"
        response
      else
        # 다중 메시지 (스레드)
        responses = send_thread_replies(acct, messages, status_id)
        puts "✅ @#{acct}에게 교무 스레드 응답 완료 (#{messages.length}개 툿)"
        responses
      end
  ...
  def self.test_connection
    begin
      ...
      puts "❌ 교무망 연결 실패: 토큰이 유효하지 않습니다"
      puts "   #{e.message}"
      false
    rescue Mastodon::Error::NotFound => e
      puts "❌ 교무망 연결 실패: 서버를 찾을 수 없습니다"
      puts "   BASE_URL: #{ENV['MASTODON_BASE_URL']}"
      puts "   #{e.message}"
      false
    rescue => e
      puts "❌ 교무망 연결 실패: #{e.message}"
      puts "   #{e.class}"
      false
    end
  end
  ...
  def self.split_long_message(message, acct)
    ...
    puts "📝 긴 교무 메시지를 분할합니다 (#{message.length}자 → 여러 툿)"
  ...
  def self.send_thread_replies(acct, messages, initial_reply_to_id)
    ...
    messages.each_with_index do |message, index|
      ...
      responses << response
      current_reply_to_id = response.id
      
      puts "   📤 교무 메시지 #{index + 1}/#{messages.length} 전송 완료"
